...
def only_thread(thread_name):
    """Runs the controlled block only if the current thread has the given name
    Otherwise skips it"""
...
def only_thread_blocking(thread_name, block_name=None):
    """Runs the controlled block only if the current thread has the given name
    Otherwise skips it. Waits for the given thread to run the block
    before allowing other threads to proceed"""
    ...
    with block_event_lock:
        ...
        block_event = block_events.setdefault(block_name,
                threading.Event())
    ...
    if thread_name == current_thread_name:
        ...
        block_event.set()
        logging.info('thread %s has set event %s', thread_name,
                     block_name)
    else:
        logging.info('thread %s will wait for event %s', thread_name,
                     block_name)
        block_event.wait()
        logging.info('thread %s has received event %s and will skip the block',
